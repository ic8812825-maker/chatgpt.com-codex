...
for token in ['Symbol', 'State', 'Level', 'FarLotBefore', 'BigLot', 'SmallLot', 'CloseFarBudget', 'ReserveAdd', 'TotalReserve', 'RecoveryPL', 'ProjectedReserveCoverage', 'FinalCloseAllowed', 'BigNetProfit', 'CloseFarLot', 'RemainingFarLot', 'ReserveCoverage']:
    assert token in logger, token
for token in ['CloseFarActualCost <= CloseFarBudget', 'Reserve does not decrease during partial Far close', 'Reserve is used only for final close']:
