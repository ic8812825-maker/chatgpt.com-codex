...
void OnStart()
{
 ...
 TestMarketEvent bigMove=Event(up.bid+(BigMoveStartPoints+5)*point,up.ask+(BigMoveStartPoints+5)*point);Drive(bigMove,8);Verify("BIG_04_HARVEST_ACTUAL_DEALS",ArraySize(SimDeals)>0&&Ctx.actualSplitHarvestNetCalculated);Verify("BIG_05_PARTIAL_CARRY",Ctx.partialFarBudgetCarry>=0&&Ctx.farLot>=0);Verify("BIG_06_RESERVE_LEDGER",ArraySize(ReserveLedger)>=0&&Ctx.totalReserve>=0);
 ...
 ResetDriver();Drive(start);TestMarketEvent partial=up;partial.partialFillRatio=.5;Drive(partial);Verify("PARTIAL_FILL_EVENT",SimCountOpenPositions()==2&&ArraySize(SimDeals)>0);TestMarketEvent finish=up;Drive(finish,2);Verify("PARTIAL_FILL_RECOVERY",State==STATE_FAR_ACTIVE||State==STATE_INITIAL_LOCK_OPENED);
