...
void OnStart()
{
 ...
 for(int phase=HARVEST_CALCULATED;phase<=HARVEST_CONSUMED;phase++){Ctx.harvestPhase=(HarvestPhase)phase;Ctx.harvestId=9001;Ctx.actualSplitHarvestNet=10;Ctx.harvestReserveAdd=6;Ctx.harvestPartialBudgetAdd=4;Ctx.harvestCarryAfter=4;State=STATE_SPLIT_BIG_HARVEST_FINAL_CHECK;SaveState();int rows=ArraySize(ReserveLedger);double expectedReserve=Ctx.totalReserve;ResetRecoveryContext();double restored=0;GetStateDouble("TotalReserve",restored);Ctx.totalReserve=restored;GetStateDouble("PartialFarBudgetCarry",restored);Ctx.partialFarBudgetCarry=restored;bool recovered=ReloadHarvestPersistence();bool continued=ContinueSplitHarvestDistribution();double once=Ctx.totalReserve;int onceRows=ArraySize(ReserveLedger);bool repeated=ContinueSplitHarvestDistribution();Verify(StringFormat("HARVEST_RESTART_%d",phase),recovered&&continued&&repeated&&Ctx.totalReserve==once&&ArraySize(ReserveLedger)==onceRows&&onceRows<=rows+1&&State==STATE_SPLIT_BIG_HARVEST_FINAL_CHECK&&Ctx.harvestId==9001);}
