...
void OnStart()
{
 ...
 for(int phase=HARVEST_LEDGER_PREPARED;phase<=HARVEST_LEDGER_WRITTEN;phase++){Ctx.harvestPhase=(HarvestPhase)phase;SaveState();HarvestPhase saved=Ctx.harvestPhase;ResetRecoveryContext();bool recovered=RecoverState();Verify(StringFormat("HARVEST_RESTART_%d",phase),recovered&&Ctx.harvestPhase==saved);}
 SmallTransitionLeg legs[5];for(int i=0;i<5;i++){legs[i].role=(SmallTransitionLegRole)i;legs[i].money=good;}legs[3].fullClose=true;legs[4].residualLot=.05;legs[2].includesOpenAndClose=true;SmallTransitionEvaluation small;Verify("SMALL_FIVE_LEGS",EvaluateSmallTransition(legs,.1,.05,.02,300,small));legs[4].role=SMALL_LEG_OLD_FAR_CLOSE;Verify("SMALL_DUPLICATE_REJECTED",!EvaluateSmallTransition(legs,.1,.05,.02,300,small));
 Verify("ACTUAL_NEW_FAR",.05<.10);Verify("SMALL_RECONCILIATION_FAIL",MathAbs(.05-.08)>VolumeMismatchToleranceLots);
 ReverseCycleProjection rc;rc.compressionRatio=.5;rc.transitionNet=5;rc.signedSwap=-1;rc.commission=1;rc.spread=1;rc.slippage=1;rc.reserveAdd=40;rc.carryAdd=0;rc.requiredMargin=10;rc.availableMargin=100;ReverseCyclesEvaluation cycles;Verify("FINITE_REVERSE_PASS",EvaluateReverseCyclesWithCosts(.1,10,1,0,1,.01,rc,cycles));rc.reserveAdd=0;rc.transitionNet=0;Verify("FINITE_REVERSE_FAIL",!EvaluateReverseCyclesWithCosts(1,100,0,0,0,.01,rc,cycles));
 FalseReverseOption options[6];for(int o=0;o<6;o++){options[o].action=(FalseReverseAction)o;options[o].projectedNet=-o;options[o].projectedRecoveryPL=-1;options[o].reserveImpact=0;options[o].projectedMarginLevel=300;options[o].remainingExposure=1;}FalseReverseEvaluation falseDecision;Verify("FALSE_REVERSE_MANUAL",!EvaluateFalseReverseMoney(options,1,falseDecision)&&falseDecision.selected==FALSE_REVERSE_MANUAL);
