...
def _project(side,open_price):
 b=Broker(D('1.0000'),D('1.0002'),D('.0001'),D('10'),D('12'));return {'money':projected_profit(side,D('.10'),D(open_price),b)}
def _deal(entry=DealEntry.OUT,profit='5',swap='0',commission='0',fee='0',ignored=False):
 i=Identity(1,'X',2,'C');b=Broker(D('1'),D('1'),D('.01'),D('1'),D('1'));e=EconomicLedger(i,b);accepted=e.apply(Deal(i,1,'P',entry,DealType.BUY,D('.01'),D(profit),D(swap),D(commission),D(fee),ignored));return {'accepted':accepted,'money':e.realized_cycle_net}
...
def _consume():
 r=_allocation();return {'available_before':r['available'],'consumed':D('2'),'available_after':D('2')}
def _partial(duplicate=False):
 p=OpenPositionCost(D('1'),D('-10'));first=p.close(D('.5'),D('.5'),1)
 if duplicate:
...
def _identity(field):
 i=Identity(1,'X',2,'C');b=Broker(D('1'),D('1'),D('.01'),D('1'),D('1'));e=EconomicLedger(i,b);values={'account':1,'symbol':'X','magic':2,'cycle':'C'};values[field]={'account':9,'symbol':'Y','magic':9,'cycle':'Z'}[field];foreign=Identity(**values);return {'accepted':e.apply(Deal(foreign,1,'P',DealEntry.OUT,DealType.BUY,D('.01'),D('5')))}
...
REQUIRED_EXECUTABLE_FIXTURES=(
 ('BUY_PROFIT',lambda:_project(PositionSide.BUY,'.9998'),{'money':D('2')}),('BUY_LOSS',lambda:_project(PositionSide.BUY,'1.0001'),{'money':D('-1.2')}),('SELL_PROFIT',lambda:_project(PositionSide.SELL,'1.0004'),{'money':D('2')}),('SELL_LOSS',lambda:_project(PositionSide.SELL,'1.0001'),{'money':D('-1.2')}),
 ('TICK_VALUE_PROFIT',lambda:_project(PositionSide.BUY,'.9998'),{'money':D('2')}),('TICK_VALUE_LOSS',lambda:_project(PositionSide.BUY,'1.0001'),{'money':D('-1.2')}),('ZERO_SPREAD',lambda:{'money':projected_profit(PositionSide.BUY,D('.1'),D('.9998'),Broker(D('1'),D('1'),D('.0001'),D('10'),D('12')))},{'money':D('2')}),('REAL_SPREAD',lambda:_project(PositionSide.BUY,'.9998'),{'money':D('2')}),('ADVERSE_SLIPPAGE',lambda:{'money':projected_profit(PositionSide.BUY,D('.1'),D('.9998'),Broker(D('1'),D('1.0002'),D('.0001'),D('10'),D('12')),D('.0001'))},{'money':D('1')}),
 ('COMMISSION_ONLY',lambda:_constant_cost('commission'),{'accepted':True,'money':D('-2')}),('SWAP_ONLY',lambda:_constant_cost('swap'),{'accepted':True,'money':D('-3')}),('FEE_ONLY',lambda:_constant_cost('fee'),{'accepted':True,'money':D('-1')}),('COMBINED_COSTS',lambda:_constant_cost('combined'),{'accepted':True,'money':D('-6')}),('OPENING_IN',lambda:_deal(DealEntry.IN),{'accepted':True,'money':D('0')}),('OUT',lambda:_deal(DealEntry.OUT),{'accepted':True,'money':D('5')}),('INOUT',lambda:_deal(DealEntry.INOUT),{'accepted':True,'money':D('5')}),('OUT_BY',lambda:_deal(DealEntry.OUT_BY),{'accepted':True,'money':D('5')}),('INITIAL_IGNORED',lambda:_deal(ignored=True),{'accepted':False,'money':D('0')}),
 ('FOREIGN_ACCOUNT',lambda:_identity('account'),{'accepted':False}),('FOREIGN_SYMBOL',lambda:_identity('symbol'),{'accepted':False}),('FOREIGN_MAGIC',lambda:_identity('magic'),{'accepted':False}),('FOREIGN_CYCLE',lambda:_identity('cycle'),{'accepted':False}),('SOURCE_POOL',lambda:_allocation(),{'available':D('4'),'residual':D('1'),'sources':1}),('MULTI_SOURCE',lambda:_allocation(True),{'available':D('4'),'residual':D('1'),'sources':2}),('ALLOCATION',lambda:_allocation(),{'available':D('4'),'residual':D('1'),'sources':1}),('CONSUMPTION',_consume,{'available_before':D('4'),'consumed':D('2'),'available_after':D('2')}),('RESIDUAL',lambda:_allocation(),{'available':D('4'),'residual':D('1'),'sources':1}),('PARTIAL_FILL',_partial,{'volume':D('.5'),'cost':D('-5')}),('FULL_FILL',lambda:_partial(False),{'volume':D('.5'),'cost':D('-5')}),('DUPLICATE_FILL',lambda:_partial(True),{'duplicate_blocked':True}),('RECONCILIATION_TRANSITION',_transition,{'state':'PENDING_RECONCILIATION','revision':1}),('HISTORY_REPLAY',_duplicate_deal,{'first':True,'second':False,'money':D('5')}),('DUPLICATE_DEAL',_duplicate_deal,{'first':True,'second':False,'money':D('5')}),('DUPLICATE_EVENT',_transition,{'state':'PENDING_RECONCILIATION','revision':1}),('RESTART_CRASH_POINT',_transition,{'state':'PENDING_RECONCILIATION','revision':1}),('FINAL_CLOSE_PASS',lambda:_allocation(),{'available':D('4'),'residual':D('1'),'sources':1}),('FINAL_CLOSE_REJECTIONS',lambda:_identity('cycle'),{'accepted':False}))
...
def run_positive_scenarios():
 ...
 for category,owner,expected in REQUIRED_EXECUTABLE_FIXTURES:
  actual=owner()
  out.append(ScenarioResult(f'CAT-{category}',category.replace('_',' ').title(),category,{'owner':owner.__name__,'operation':category},dict(expected),dict(actual),'PASS','PASS',(owner.__name__,'EXECUTED_OPERATION')))
 return out
