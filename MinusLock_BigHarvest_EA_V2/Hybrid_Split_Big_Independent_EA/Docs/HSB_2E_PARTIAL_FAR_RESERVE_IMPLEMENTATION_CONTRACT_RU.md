# HSB.2E PARTIAL_FAR_RESERVE implementation contract

- **INPUT DTO:** `HSBI_PartialFarRequest` with account, symbol, magic, cycleId, stateRevision, actionId and immutable snapshots.
- **OUTPUT DECISION DTO:** `HSBI_PartialFarDecision` with status, reason, intents, requiredNextState and digest.
...
- **PRECONDITIONS:** real close-Far budget positive.
- **FORMULAS:** partial close uses close-Far share only; money includes commission, swap, fee, spread and slippage.
...
- **LEDGER ALLOCATION:** only confirmed real-deal net; Reserve unchanged.
