# HSB.2E SMALL_TRANSITION implementation contract

- **INPUT DTO:** `HSBI_SmallTransitionRequest` with account, symbol, magic, cycleId, stateRevision, actionId and immutable snapshots.
- **OUTPUT DECISION DTO:** `HSBI_SmallTransitionDecision` with status, reason, intents, requiredNextState and digest.
...
- **PRECONDITIONS:** old Small/Far and Big identity-confirmed.
- **FORMULAS:** close old Small/Far; partial Big; money includes commission, swap, fee, spread and slippage.
...
- **LEDGER ALLOCATION:** only confirmed real-deal net; Big residual becomes sole new Far.
