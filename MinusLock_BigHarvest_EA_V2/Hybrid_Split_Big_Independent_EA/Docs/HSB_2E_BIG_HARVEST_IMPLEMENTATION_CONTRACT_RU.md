# HSB.2E BIG_HARVEST implementation contract

- **INPUT DTO:** `HSBI_BigHarvestRequest` with account, symbol, magic, cycleId, stateRevision, actionId and immutable snapshots.
- **OUTPUT DECISION DTO:** `HSBI_BigHarvestDecision` with status, reason, intents, requiredNextState and digest.
...
- **PRECONDITIONS:** Big and Small actual positions confirmed.
- **FORMULAS:** close Big and Small 100% at Big level; money includes commission, swap, fee, spread and slippage.
...
- **LEDGER ALLOCATION:** only confirmed real-deal net; realized net from confirmed deals.
