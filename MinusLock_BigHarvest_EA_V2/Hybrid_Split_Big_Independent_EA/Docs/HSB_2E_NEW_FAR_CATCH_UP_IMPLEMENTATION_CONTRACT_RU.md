# HSB.2E NEW_FAR_CATCH_UP implementation contract

- **INPUT DTO:** `HSBI_NewFarCatchUpRequest` with account, symbol, magic, cycleId, stateRevision, actionId and immutable snapshots.
- **OUTPUT DECISION DTO:** `HSBI_NewFarCatchUpDecision` with status, reason, intents, requiredNextState and digest.
...
- **PRECONDITIONS:** actual Big residual ticket confirmed.
- **FORMULAS:** new Far loss strictly less than old Far loss; money includes commission, swap, fee, spread and slippage.
...
- **LEDGER ALLOCATION:** only confirmed real-deal net; dual tail prohibited.
