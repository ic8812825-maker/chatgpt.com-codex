...
bool HSBI_TicketDoesNotDefineOwnership(const HSBI_PositionDescriptor &expected,const HSBI_PositionDescriptor &actual)
{
   return HSBI_SamePositionOwner(expected.identity,actual.identity)&&expected.identifier==actual.identifier&&expected.role==actual.role;
}
