...
ulong HSBI_TestDigest(const HSBI_SnapshotRecord &s){string v=IntegerToString(s.schemaVersion)+"|"+IntegerToString((int)s.cycleId)+"|"+IntegerToString((int)s.stateRevision)+"|"+s.candidatePlanDigest;ulong h=2166136261;for(int i=0;i<StringLen(v);i++){h^=(uchar)StringGetCharacter(v,i);h*=16777619;}return h;}
#endif