#!/usr/bin/env python3
"""R4-R7 broker contour: immutable evidence with position/intent binding."""
...
class HSBI_QuoteSnapshot:
    schemaVersion:int;accountLogin:int;symbol:str;magic:int;cycleId:str;stateRevision:int;snapshotId:str;snapshotRevision:int;bid:Decimal;ask:Decimal;timestamp:int;quoteSource:str='TRUSTED_TERMINAL_SNAPSHOT';digestValue:str=''
...
PRICE_AUTHORITIES={'HSBI-PRICE-R7':'TRUSTED_TERMINAL_SNAPSHOT'}
def validate_snapshot_context(snapshot,context):
    if snapshot.digestValue!=digest(snapshot.body()):return 'SNAPSHOT_DIGEST_INVALID'
    bindings={'accountLogin':'accountLogin','symbol':'symbol','magic':'magic','cycleId':'cycleId','stateRevision':'stateRevision','snapshotId':'snapshotId','snapshotRevision':'snapshotRevision'}
    if any(getattr(snapshot,a)!=context.get(b) for a,b in bindings.items()):return 'SNAPSHOT_CONTEXT_IDENTITY_MISMATCH'
    if snapshot.schemaVersion!=1 or snapshot.quoteSource!='TRUSTED_TERMINAL_SNAPSHOT':return 'SNAPSHOT_SOURCE_INVALID'
    if not snapshot.bid.is_finite() or not snapshot.ask.is_finite() or snapshot.bid<=0 or snapshot.ask<snapshot.bid:return 'SNAPSHOT_PRICE_INVALID'
    return None
...
def price_bounds(snapshot,policy,direction):
    ...
    if policy.buyCloseSide!='BID' or policy.sellCloseSide!='ASK':raise ValueError('NORMATIVE_CLOSE_SIDE_MISMATCH')
    if PRICE_AUTHORITIES.get(policy.normativeSourceId)!=policy.quoteSource or policy.quoteSource!=snapshot.quoteSource:raise ValueError('PRICE_POLICY_AUTHORITY_INVALID')
    if policy.deviationTicks<0 or policy.validFrom>policy.validUntil:raise ValueError('PRICE_POLICY_DOMAIN_INVALID')
...
def validate_binding(record,context,positions,intents,snapshot,policy):
    ...
    snapshot_error=validate_snapshot_context(snapshot,context)
