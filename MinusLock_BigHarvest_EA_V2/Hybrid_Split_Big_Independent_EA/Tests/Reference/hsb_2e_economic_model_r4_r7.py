#!/usr/bin/env python3
"""Sealed R4-R7 economic policy and money/volume-conserving settlement."""
from dataclasses import dataclass,asdict
from decimal import Decimal,ROUND_DOWN
from hsb_2e_provenance_model_r4_r7 import D,digest,grid
...
def build_economic_proposal(scenario,broker,p,positions,full_fill):
    ...
    if scenario=='INITIAL':
        ...
        before=D(big['authoritativeVolume']);closed=D(broker['volumeByTicket'].get(str(big['ticket']),0));new_volume=down_volume(before-closed,p);new_ticket=big['ticket'];far_after=new_volume;dual=old.get('remainingVolume','0') not in ('0',0,D(0))
        if before != closed + new_volume:return None,'BIG_VOLUME_CONSERVATION_FAILED'
