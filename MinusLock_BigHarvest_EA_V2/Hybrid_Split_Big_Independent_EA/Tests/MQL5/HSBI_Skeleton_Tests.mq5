...
#include "../../Include/Runtime/HSBI_RuntimeTransactionBarrier.mqh"
...
HSBI_RuntimeDecisionContext RuntimeDecision(){HSBI_RuntimeDecisionContext x;ZeroMemory(x);x.accountLogin=1;x.symbol="TEST";x.magic=7;x.cycleId=11;x.planId=5;x.stateRevision=4;x.schemaVersion=HSBI_SCHEMA_VERSION;x.moneyStateVersion=HSBI_MONEY_STATE_VERSION;x.runtimeMode=HSBI_RUNTIME_PRODUCTION;x.state=HSBI_STATE_CANDIDATE_PLANNED;x.reconciliationStatus=1;x.eventId=12;x.actionId=50;x.actualResidual=Position(HSBI_ROLE_BIG_CORE,77,88,0.5);x.actualResidual.identity.accountLogin=1;x.actualResidual.identity.symbol="TEST";x.actualResidual.identity.magic=7;x.actualResidual.identity.cycleId=11;x.actualResidual.direction=HSBI_DIRECTION_BUY;x.aggregate.valid=true;x.aggregate.levelCount=2;x.aggregate.evaluatedLevelCount=2;x.aggregate.runtimeConfirmed=true;x.aggregate.allMoneyProofsValid=true;x.aggregate.allMarginProofsValid=true;x.aggregate.allRiskProofsValid=true;x.aggregate.aggregateDigest="AGG";x.candidate.normalizedVolume=0.2;x.candidate.oldFarVolume=1.0;x.candidate.compressionLots=0.8;x.candidate.sourceBigCoreIdentifier=77;x.candidate.sourceBigCoreTicket=88;x.candidate.validationStatus=HSBI_STATUS_VALID;x.candidate.nextCycleFeasible=true;x.candidate.moneyProofValid=true;x.candidate.marginProofValid=true;x.candidate.riskProofValid=true;x.candidate.futureSmallProofValid=true;x.candidate.catchUpProofValid=true;x.candidate.aggregateProofValid=true;x.candidate.aggregateRuntimeConfirmed=true;x.candidate.aggregateProofDigest="AGG";x.candidate.allocationPolicyValid=true;x.candidate.controlSnapshotsValid=true;x.candidate.costSnapshotsValid=true;x.candidate.fullDigestValid=true;x.candidate.moneyProofDigest="M";x.candidate.marginProofDigest="G";x.candidate.riskProofDigest="R";x.candidate.futureSmallProofDigest="F";x.candidate.catchUpProofDigest="C";x.candidate.allocationPolicyDigest="A";x.candidate.controlPriceDigest="P";x.candidate.costSnapshotDigest="K";x.candidate.candidateDigest="N";x.catchUp.valid=true;x.catchUp.runtimeConfirmed=true;x.catchUp.reserveSourceIdentityValid=true;x.catchUp.farLossSourceIdentityValid=true;x.allocationPolicy=Policy();x.allocationSource=AllocationSource();x.consumptionKey=Consumption();x.moneyIdentity=ProofIdentity(HSBI_ROLE_BIG_CORE,HSBI_DIRECTION_BUY,77,9,10);x.riskIdentity=x.moneyIdentity;x.marginIdentity=x.moneyIdentity;x.immutable=true;x.marketFresh=true;x.costFresh=true;x.reconciliationConfirmed=true;x.positionActuallyRead=true;x.ownershipConfirmed=true;x.residualActual=true;x.persistencePrepared=true;x.moneyRuntimeConfirmed=true;x.riskRuntimeConfirmed=true;x.marginRuntimeConfirmed=true;x.inputDigest=HSBI_RuntimeDecisionContextDigest(x);return x;}
HSBI_RuntimeBarrierInput RuntimeBarrier(){HSBI_RuntimeBarrierInput b;ZeroMemory(b);b.context=RuntimeDecision();b.expectedEventId=11;b.expectedActionId=50;b.expectedPlanId=5;b.expectedCycleId=11;b.expectedStateRevision=4;b.expectedDigest=b.context.inputDigest;b.snapshotsFresh=true;b.reconciliationAllowed=true;b.positionRead=true;b.ticketConfirmed=true;b.volumeConfirmed=true;b.directionConfirmed=true;b.ownershipConfirmed=true;b.moneyConfirmed=true;b.marginConfirmed=true;b.riskConfirmed=true;b.persistencePrepared=true;b.payloadDigest="PAYLOAD";return b;}

HSBI_RestartedRuntimeState RestartState(){HSBI_RestartedRuntimeState s;ZeroMemory(s);s.current=RuntimeDecision();s.persisted=StateSnapshot();s.expectedPlanId=5;s.expectedStateRevision=4;s.expectedEventId=12;s.expectedActionId=50;s.persistedResidual=s.current.actualResidual;s.persistedAllocation=s.current.allocationSource;s.persistedConsumption=s.current.consumptionKey;s.snapshotPresent=true;s.historyUnchanged=true;s.brokerMoneyConfirmed=true;s.reconciliationConfirmed=true;s.persistedDigest=s.persisted.snapshotDigest;return s;}
...
void OnStart()
{
   ...
   HSBI_RuntimeDecisionContext rd=RuntimeDecision();Check("T431","HSBI-2D-CTX",true,HSBI_ValidateRuntimeDecisionContext(rd,1,"TEST",7,11,5,4).valid,"VALID_CONTEXT_NO_TRADE");
   rd=RuntimeDecision();Check("T432","HSBI-2D-ID",false,HSBI_ValidateRuntimeDecisionContext(rd,2,"TEST",7,11,5,4).valid,"FOREIGN_ACCOUNT");
   rd=RuntimeDecision();Check("T433","HSBI-2D-ID",false,HSBI_ValidateRuntimeDecisionContext(rd,1,"OTHER",7,11,5,4).valid,"FOREIGN_SYMBOL");
   rd=RuntimeDecision();Check("T434","HSBI-2D-ID",false,HSBI_ValidateRuntimeDecisionContext(rd,1,"TEST",8,11,5,4).valid,"FOREIGN_MAGIC");
   rd=RuntimeDecision();Check("T435","HSBI-2D-REV",false,HSBI_ValidateRuntimeDecisionContext(rd,1,"TEST",7,11,5,5).valid,"REVISION_MISMATCH");
   rd=RuntimeDecision();rd.schemaVersion++;rd.inputDigest=HSBI_RuntimeDecisionContextDigest(rd);Check("T436","HSBI-2D-SCHEMA",false,HSBI_ValidateRuntimeDecisionContext(rd,1,"TEST",7,11,5,4).valid,"SCHEMA_MISMATCH");
   rd=RuntimeDecision();rd.moneyStateVersion++;rd.inputDigest=HSBI_RuntimeDecisionContextDigest(rd);Check("T437","HSBI-2D-MONEY-VERSION",false,HSBI_ValidateRuntimeDecisionContext(rd,1,"TEST",7,11,5,4).valid,"MONEY_VERSION_MISMATCH");
   rd=RuntimeDecision();rd.marketFresh=false;rd.inputDigest=HSBI_RuntimeDecisionContextDigest(rd);Check("T438","HSBI-2D-FRESH",true,HSBI_ValidateRuntimeDecisionContext(rd,1,"TEST",7,11,5,4).status==HSBI_DECISION_STALE,"STALE_MARKET");
   rd=RuntimeDecision();rd.reconciliationConfirmed=false;rd.inputDigest=HSBI_RuntimeDecisionContextDigest(rd);Check("T439","HSBI-2D-RECON",true,HSBI_ValidateRuntimeDecisionContext(rd,1,"TEST",7,11,5,4).status==HSBI_DECISION_RECONCILIATION_REQUIRED,"MISSING_RECONCILIATION");
   rd=RuntimeDecision();rd.aggregate.levelCount=1;rd.inputDigest=HSBI_RuntimeDecisionContextDigest(rd);Check("T440","HSBI-2D-FS",false,HSBI_ValidateRuntimeDecisionContext(rd,1,"TEST",7,11,5,4).valid,"INCOMPLETE_AGGREGATE");
   rd=RuntimeDecision();rd.residualActual=false;rd.inputDigest=HSBI_RuntimeDecisionContextDigest(rd);Check("T441","HSBI-2D-RESIDUAL",false,HSBI_ValidateRuntimeDecisionContext(rd,1,"TEST",7,11,5,4).valid,"PROJECTED_RESIDUAL");
   rd=RuntimeDecision();rd.candidate.sourceBigCoreIdentifier=99;rd.inputDigest=HSBI_RuntimeDecisionContextDigest(rd);Check("T442","HSBI-2D-NF",false,HSBI_ValidateRuntimeDecisionContext(rd,1,"TEST",7,11,5,4).valid,"FOREIGN_CANDIDATE");
   rd=RuntimeDecision();rd.moneyRuntimeConfirmed=false;rd.inputDigest=HSBI_RuntimeDecisionContextDigest(rd);Check("T443","HSBI-2D-MONEY",false,HSBI_ValidateRuntimeDecisionContext(rd,1,"TEST",7,11,5,4).valid,"MONEY_UNAVAILABLE");
   rd=RuntimeDecision();rd.riskRuntimeConfirmed=false;rd.inputDigest=HSBI_RuntimeDecisionContextDigest(rd);Check("T444","HSBI-2D-RISK",false,HSBI_ValidateRuntimeDecisionContext(rd,1,"TEST",7,11,5,4).valid,"PROXY_RISK_BLOCKED");
   rd=RuntimeDecision();rd.allocationSource.alreadyConsumed=31.0;rd.inputDigest=HSBI_RuntimeDecisionContextDigest(rd);Check("T445","HSBI-2D-ALLOC",false,HSBI_ValidateRuntimeDecisionContext(rd,1,"TEST",7,11,5,4).valid,"ALLOCATION_OVERRUN");
   rd=RuntimeDecision();rd.consumptionConflict=true;rd.inputDigest=HSBI_RuntimeDecisionContextDigest(rd);Check("T446","HSBI-2D-CONSUME",true,HSBI_ValidateRuntimeDecisionContext(rd,1,"TEST",7,11,5,4).status==HSBI_DECISION_CONFLICT,"CONSUMPTION_CONFLICT");
   rd=RuntimeDecision();rd.inputDigest="BAD";Check("T447","HSBI-2D-DIGEST",false,HSBI_ValidateRuntimeDecisionContext(rd,1,"TEST",7,11,5,4).valid,"DIGEST_MISMATCH");
   rd=RuntimeDecision();rd.persistencePrepared=false;rd.inputDigest=HSBI_RuntimeDecisionContextDigest(rd);Check("T448","HSBI-2D-PERSIST",true,HSBI_ValidateRuntimeDecisionContext(rd,1,"TEST",7,11,5,4).status==HSBI_DECISION_PERSISTENCE_REQUIRED,"PERSISTENCE_REQUIRED");
   HSBI_RuntimeBarrierInput rb=RuntimeBarrier();Check("T449","HSBI-2D-BARRIER",true,HSBI_CanAdvanceRuntimeDecision(rb).valid,"VALID_ADVANCE_NO_TRADE");
   rb=RuntimeBarrier();rb.expectedEventId=12;Check("T450","HSBI-2D-BARRIER-EVENT",false,HSBI_CanAdvanceRuntimeDecision(rb).valid,"STALE_EVENT");
   rb=RuntimeBarrier();rb.expectedActionId=51;Check("T451","HSBI-2D-BARRIER-ACTION",false,HSBI_CanAdvanceRuntimeDecision(rb).valid,"WRONG_ACTION");
   rb=RuntimeBarrier();rb.completedOutcome=true;rb.lastCompletedActionId=50;rb.lastCompletedPayloadDigest="PAYLOAD";Check("T452","HSBI-2D-BARRIER-NOOP",true,HSBI_CanAdvanceRuntimeDecision(rb).status==HSBI_DECISION_NO_OP,"COMPLETED_RETRY_NOOP");
   rb=RuntimeBarrier();rb.retryRequested=true;Check("T453","HSBI-2D-BARRIER",true,HSBI_CanAdvanceRuntimeDecision(rb).valid,"SAME_ACTION_RETRY");
   rb=RuntimeBarrier();rb.persistencePrepared=false;Check("T454","HSBI-2D-BARRIER-PERSIST",true,HSBI_CanAdvanceRuntimeDecision(rb).status==HSBI_DECISION_PERSISTENCE_REQUIRED,"BARRIER_PERSISTENCE_REQUIRED");
   HSBI_RestartedRuntimeState restart=RestartState();Check("T455","HSBI-2D-RESTART",true,HSBI_ValidateRestartedRuntimeState(restart).valid,"VALID_RESTART_NO_TRADE");
   restart=RestartState();restart.current.stateRevision=5;restart.current.inputDigest=HSBI_RuntimeDecisionContextDigest(restart.current);Check("T456","HSBI-2D-RESTART-ID",false,HSBI_ValidateRestartedRuntimeState(restart).valid,"ALTERED_REVISION");
   restart=RestartState();restart.current.planId=6;restart.current.inputDigest=HSBI_RuntimeDecisionContextDigest(restart.current);Check("T457","HSBI-2D-RESTART-ID",false,HSBI_ValidateRestartedRuntimeState(restart).valid,"ALTERED_PLAN");
   restart=RestartState();restart.persistedResidual.ticket=99;Check("T458","HSBI-2D-RESTART-POS",false,HSBI_ValidateRestartedRuntimeState(restart).valid,"ALTERED_TICKET");
   restart=RestartState();restart.persistedResidual.actualVolume=0.4;Check("T459","HSBI-2D-RESTART-POS",false,HSBI_ValidateRestartedRuntimeState(restart).valid,"ALTERED_VOLUME");
   restart=RestartState();restart.sourceReused=true;Check("T460","HSBI-2D-RESTART-HISTORY",true,HSBI_ValidateRestartedRuntimeState(restart).status==HSBI_DECISION_CONFLICT,"REUSED_SOURCE");
   restart=RestartState();restart.duplicateConsumption=true;Check("T461","HSBI-2D-RESTART-NOOP",true,HSBI_ValidateRestartedRuntimeState(restart).status==HSBI_DECISION_NO_OP,"DUPLICATE_CONSUMPTION_NOOP");
   restart=RestartState();restart.payloadConflict=true;Check("T462","HSBI-2D-RESTART-HISTORY",true,HSBI_ValidateRestartedRuntimeState(restart).status==HSBI_DECISION_CONFLICT,"CONFLICTING_PAYLOAD");
   restart=RestartState();restart.snapshotPresent=false;Check("T463","HSBI-2D-RESTART",true,HSBI_ValidateRestartedRuntimeState(restart).status==HSBI_DECISION_PERSISTENCE_REQUIRED,"MISSING_SNAPSHOT");
   restart=RestartState();restart.unresolvedPending=true;Check("T464","HSBI-2D-RESTART-PENDING",true,HSBI_ValidateRestartedRuntimeState(restart).status==HSBI_DECISION_PERSISTENCE_REQUIRED,"UNRESOLVED_PENDING");
