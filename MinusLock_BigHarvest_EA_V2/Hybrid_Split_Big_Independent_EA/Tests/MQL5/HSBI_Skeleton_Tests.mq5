...
void OnStart()
{
   ...
   HSBI_ExecutionJournalEntry jgap=Journal(4,j1.currentEntryDigest,"P4");Check("T306","HSBI-JOURNAL-001",true,HSBI_ClassifyJournalAppend(jgap,j1,j1.currentEntryDigest,11,5,4)==HSBI_JOURNAL_APPENDED,"MONOTONIC_APPEND");
   ...
   ss=StateSnapshot();ss.completedIntent=Intent();ss.completedActionCount=1;ss.snapshotDigest=HSBI_ExecutionStateSnapshotDigest(ss);Check("T319","HSBI-PERSIST-002",true,HSBI_RecoverExecutionState(ss,1,1,1,"TEST",7,11,4)==HSBI_RECOVERY_ACCEPTED,"RESTART_COMPLETED");
   intent=Intent();HSBI_ExternalTransactionOutcome out=Outcome(intent);HSBI_ExecutionReconciliationInput ri;ZeroMemory(ri);ri.intent=intent;ri.outcome=out;ri.lastAppliedEventId=10;ri.snapshotFresh=true;ri.ownershipPassed=true;ri.volumeTolerance=1e-8;ri.priceTolerance=0.00001;Check("T320","HSBI-RECON-002",true,HSBI_ReconcileExecutionOutcome(ri).completionAllowed,"VALID_OUTCOME");
