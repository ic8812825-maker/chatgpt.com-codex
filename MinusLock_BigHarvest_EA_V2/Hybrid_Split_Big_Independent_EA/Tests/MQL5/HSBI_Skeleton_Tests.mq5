...
HSBI_BasketMoneyResult InjectedBasket(const double recovery,const double margin,const double exposure,const double transition){HSBI_BasketMoneyResult b;ZeroMemory(b);b.status=HSBI_CALC_PASS;b.valid=true;b.brokerRuntimeConfirmed=true;b.basketNetMoney=recovery;b.recoveryMoney=recovery;b.totalMargin=margin;b.grossExposure=exposure;b.transitionLoss=transition;b.safetyBuffer=1.0;b.reason=HSBI_REASON_OK;b.details="INJECTED_BROKER_CONTRACT";b.core.status=HSBI_CALC_PASS;b.core.valid=true;b.core.projected=true;b.core.netMoney=recovery;b.far.status=HSBI_CALC_PASS;b.far.valid=true;b.far.projected=true;b.far.netMoney=-transition;return b;}
...
void OnStart()
{
   ...
   HSBI_BrokerMoneyEvaluationResult reserveProof;ZeroMemory(reserveProof);reserveProof.status=HSBI_CALC_PASS;reserveProof.valid=true;reserveProof.projected=true;reserveProof.netMoney=30.0;HSBI_BrokerMoneyEvaluationResult lossProof=reserveProof;lossProof.netMoney=-5.0;
   HSBI_ReserveCatchUpInput rci;ZeroMemory(rci);rci.allocationPolicy=Policy();rci.reserveEligibleMoney=30.0;rci.farLossIncreaseMoney=5.0;rci.executionSafetyBuffer=2.0;rci.netBigVolume=3.0;rci.farVolume=1.0;rci.farDirection=HSBI_DIRECTION_SELL;rci.reserveSourceProof=reserveProof;rci.farLossProof=lossProof;rci.sourceDealId=9;rci.sourceEventId=10;rci.planId=5;rci.stateRevision=4;rci.snapshotId=1;rci.projected=true;rci.moneyAvailable=true;rci.fresh=true;
   ...
   rci.allocationPolicy=Policy();rci.reserveEligibleMoney=-1.0;rci.farLossIncreaseMoney=1.0;rci.executionSafetyBuffer=0.1;rci.netBigVolume=3.0;rci.farVolume=1.0;rci.reserveSourceProof=reserveProof;rci.farLossProof=lossProof;rci.planId=5;rci.stateRevision=4;rci.snapshotId=1;rci.moneyAvailable=true;rci.fresh=true;Check("T167","HSBI-CATCHUP-002",true,!HSBI_EvaluateReserveCatchUp(rci).valid,"NEGATIVE_SOURCE_REJECT");
