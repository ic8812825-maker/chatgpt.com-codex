...
def run(root):
 ...
 added=subprocess.check_output(['git','diff','--unified=0',f'{BASE}..HEAD','--','*.mq5','*.mqh'],cwd=root,text=True);check('R7_BROKER_DISPATCH_DISABLED',added=='')
 check('R7_REAL_MODEL_MUTATION_SENSITIVITY',(root/'Tests/Static/run_hsb_2e_r4_r7_semantic_mutations.py').exists())
 seal=subprocess.run(['python3',str(root/'Tests/Static/verify_hsb_2e_r4_r7_evidence_seal.py'),'--root',str(root)],capture_output=True,text=True);check('R7_MANIFEST_AND_SEAL',seal.returncode==0)
 docs=('README_RU.md','BUILD_INFO.md','PROJECT_MAP_RU.md','CHANGELOG_RU.md','Docs/19_REQUIREMENT_TRACEABILITY_MATRIX_RU.md','Docs/21_PRODUCTION_READINESS_CRITERIA_RU.md','Docs/22_OPEN_DECISIONS_REGISTER_RU.md');check('R7_CANONICAL_STATUS',all((root/f).read_text().count('HSB_2E_PREP_R4_R7_CANONICAL_STATUS_BEGIN')==1 and 'TRADING_LOGIC_START_ALLOWED=YES' not in (root/f).read_text() for f in docs))
