...
def apply(m,root):
 ...
 if m['mutation_type']=='semantic':rehash(root/'Reports/HSB_2D_V1_R5_FILE_MANIFEST_SHA256.txt',m['target'],root)
def parse(stdout):
 failed=[];statuses={}
...
def selftests(catalog):
 ...
 cases['MR008']=not q.exists();cases['MR009']=len(ids)==len(set(ids));cases['MR010']=set(ids)==set(f'M{i:03}' for i in range(1,151))
 return [{'id':k,'status':'PASS' if v else 'FAIL'} for k,v in cases.items()]
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--root',required=True);ap.add_argument('--catalog',default='Tests/Static/hsb_2d_v1_r5_mutations.json');ap.add_argument('--output-json');ap.add_argument('--output-text');ap.add_argument('--publish-evidence',action='store_true');a=ap.parse_args();root=Path(a.root).resolve();catalog=json.loads((root/a.catalog).read_text(encoding='utf-8'))
 publish_json=root/'Tests/Evidence/HSB_2D_V1_R5_MUTATION_RESULTS.json';publish_text=root/'Tests/Evidence/HSB_2D_V1_R5_MUTATION_RESULTS.txt';outj=Path(a.output_json).resolve() if a.output_json else (publish_json if a.publish_evidence else None);outt=Path(a.output_text).resolve() if a.output_text else (publish_text if a.publish_evidence else None)
 ...
 verifier=root/'Tests/Static/verify_hsb_2d_v1_r5.py';clean=subprocess.run([sys.executable,str(verifier),'--root',str(root)],text=True,capture_output=True)
 ...
 for m in required_mutations:
  ...
  try:
   with tempfile.TemporaryDirectory(prefix='hsbi-r2-') as td:
    ...
    if m['mutation_type']=='semantic':allowed.add('Reports/HSB_2D_V1_R5_FILE_MANIFEST_SHA256.txt')
    rec['changed_files']=changed;rec['unexpected_changed_files']=sorted(set(changed)-allowed);rec['applied']=m['target'] in changed
    cp=subprocess.run([sys.executable,str(copy/'Tests/Static/verify_hsb_2d_v1_r5.py'),'--root',str(copy),'--fixture-mode'],text=True,capture_output=True);rec['verifier_started']=True;rec['verifier_exit_code']=cp.returncode;rec['verifier_crashed']=cp.returncode not in (0,1);failed,status=parse(cp.stdout);rec['actual_failed_checks']=failed;rec['manifest_check_status']=status.get('S045','MISSING');rec['stdout_sha256']=hashlib.sha256(cp.stdout.encode()).hexdigest();rec['stderr_sha256']=hashlib.sha256(cp.stderr.encode()).hexdigest();expected=all(x in failed for x in m['expected_check_ids']);manifest_only=failed==['S045'] and 'S045' not in m['expected_check_ids'];rec['primary_failure_check']=failed[0] if failed else '';rec['secondary_failure_checks']=failed[1:];rec['manifest_only_failure']=manifest_only
 ...
 lines+=['HSB_2D_V1_R5_MUTATION_SUMMARY']+[f'{k}={v}' for k,v in summary.items()]+[f'RUNNER_SELF_TESTS={sum(x["status"]=="PASS" for x in selfcheck)}/10',f"RESULT={'PASS' if ok else 'FAIL'}"]
 text='\n'.join(lines)+'\n';
