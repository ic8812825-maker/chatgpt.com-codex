...
def main():
 ...
 root=Path(a.root).resolve();catalog=json.loads((root/'Tests/Static/hsb_2e_prep_r4_r2_mutations.json').read_text())['mutations']
 ...
 with tempfile.TemporaryDirectory(prefix='r4-r2-mut-') as td:
  ...
  for mutation in catalog:
   ...
   command=[sys.executable,str(fixture/'Tests/Static/verify_hsb_2e_prep_r4_r2.py'),'--root',str(fixture),'--fixture-mode']
 ...
 if a.evidence_dir:
  ...
  (directory/'HSB_2E_PREP_R4_R2_MUTATION_RESULTS.json').write_text(json.dumps(summary,indent=2,sort_keys=True)+'\n')
  (directory/'HSB_2E_PREP_R4_R2_MUTATION_RESULTS.txt').write_text(output)
  (directory/'HSB_2E_PREP_R4_R2_MUTATION_UNIQUENESS_AUDIT.json').write_text(json.dumps({k:v for k,v in summary.items() if k!='results'},indent=2,sort_keys=True)+'\n')
