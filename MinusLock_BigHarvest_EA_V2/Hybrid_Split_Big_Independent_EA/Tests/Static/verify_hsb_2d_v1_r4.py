...
from hsb_mql5_lexer import (prove_top_level_guard,prove_reject_constructor,
 prove_unique_final_success,active_compact,lexer_self_tests,LexerError)
BASELINE='ea4047752e5c214a4201eb23fa6ddc62043d1167';PREFIX='MinusLock_BigHarvest_EA_V2/Hybrid_Split_Big_Independent_EA/'
...
REQUIRED={'CURRENT_STAGE':'HSB.2D-V1-R4','HSB.2D_V1_R1_PREVIOUS_ACCEPTANCE':'HISTORICAL_SUPERSEDED','HSB.2D_V1_R2_PREVIOUS_ACCEPTANCE':'HISTORICAL_SUPERSEDED','HSB.2D_V1_R3_PREVIOUS_ACCEPTANCE':'HISTORICAL_SUPERSEDED','HSB.2D_V1_R4':'CORRECTED_OFFLINE_VERIFICATION','COMMENT_AWARE_MQL5_ANALYSIS':'PASS','PREPROCESSOR_COMMENT_ORDER':'PASS','GUARD_REACHABILITY_PROOF':'PASS','GUARD_DOMINANCE_PROOF':'PASS','REJECT_CONSTRUCTOR_PROOF':'PASS','UNIQUE_FINAL_SUCCESS_PROOF':'PASS','METAEDITOR_MAIN_COMPILE':'NOT_EXECUTED_MT5_UNAVAILABLE','METAEDITOR_TEST_COMPILE':'NOT_EXECUTED_MT5_UNAVAILABLE','MQL5_TESTS_T01_T464':'NOT_EXECUTED_MT5_UNAVAILABLE','BROKER_MONEY_RUNTIME_PROOF':'NOT_EXECUTED_MT5_UNAVAILABLE','HSB.2D_V2':'AWAITING_ADMIN_REVIEW','HSB.2E':'NOT_STARTED','TRADING_IMPLEMENTED':'NO','BROKER_DISPATCH_IMPLEMENTED':'NO','TRADE_REQUESTS_ALLOWED':'NO','REAL_TRADING_ALLOWED':'NO'}
SEAL_FILES=('Tests/Evidence/HSB_2D_V1_R4_CLEAN_RESULT.txt','Tests/Evidence/HSB_2D_V1_R4_MUTATION_RESULTS.json','Tests/Evidence/HSB_2D_V1_R4_MUTATION_RESULTS.txt','Tests/Static/verify_hsb_2d_v1_r4.py','Tests/Static/run_hsb_2d_v1_r4_mutations.py','Tests/Static/hsb_2d_v1_r4_mutations.json','Tests/Static/hsb_mql5_lexer.py','Reports/HSB_2D_V1_R4_CONDITIONAL_EARLY_SUCCESS_ANALYSIS_RU.md','Reports/HSB_2D_V1_R4_CONTROL_FLOW_ARCHITECTURE_RU.md','Tests/Evidence/HSB_2D_V1_R4_GUARD_PROOFS.json')
...
def expected_files(r):
 f={'Hybrid_Split_Big_Independent_EA.mq5','Tests/MQL5/HSBI_Skeleton_Tests.mq5','Tests/Static/verify_hsb_2d_v1_r4.py','Tests/Static/run_hsb_2d_v1_r4_mutations.py','Tests/Static/hsb_2d_v1_r4_mutations.json','Tests/Static/hsb_mql5_lexer.py',*STATUS_DOCUMENTS}
 f|={p.relative_to(r).as_posix() for p in r.glob('Include/**/*.mqh')};f|={p.relative_to(r).as_posix() for p in r.glob('Reports/HSB_2D_V1_R4_*.md') if 'FINAL_PUBLICATION_RECORD' not in p.name};return f
...
def status(r):
 ...
 for rel in STATUS_DOCUMENTS:
  s=(r/rel).read_text(encoding='utf-8-sig');bm='HSB_2D_V1_R4_CANONICAL_STATUS_BEGIN';em='HSB_2D_V1_R4_CANONICAL_STATUS_END';meta[rel]=(s.count(bm),s.count(em));
...
def run(root,fixture=False,skip_seal=False):
 ...
 mok,md,mm=check_list(r,'Reports/HSB_2D_V1_R4_FILE_MANIFEST_SHA256.txt',expected_files(r));add('S045',mok,md);sok,sd,sm=check_list(r,'Tests/Evidence/HSB_2D_V1_R4_EVIDENCE_SEAL_SHA256.txt',set(SEAL_FILES));sm=({k:0 for k in ('MISSING','EXTRA','DUPLICATES','EXTERNAL_PATHS','HASH_MISMATCHES')} if skip_seal else sm);add('S046E',sok or skip_seal,'evidence seal verified' if sok or skip_seal else sd);lex=lexer_self_tests();add('SLEX36',len(lex)>=36 and all(lex.values()),f'passed={sum(lex.values())}/{len(lex)}')
