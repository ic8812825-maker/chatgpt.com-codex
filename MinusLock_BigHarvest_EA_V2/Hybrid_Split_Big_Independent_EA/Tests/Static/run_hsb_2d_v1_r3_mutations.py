...
def apply(m,root):
 ...
 if m['mutation_type']=='semantic':rehash(root/'Reports/HSB_2D_V1_R3_FILE_MANIFEST_SHA256.txt',m['target'],root)
def parse(stdout):
 failed=[];statuses={}
...
def selftests(catalog):
 ...
 cases['MR008']=not q.exists();cases['MR009']=len(ids)==len(set(ids));cases['MR010']=set(f'M{i:03}' for i in range(1,124))<=set(ids)
 return [{'id':k,'status':'PASS' if v else 'FAIL'} for k,v in cases.items()]
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--root',required=True);ap.add_argument('--catalog',default='Tests/Static/hsb_2d_v1_r3_mutations.json');ap.add_argument('--output-json');ap.add_argument('--output-text');ap.add_argument('--publish-evidence',action='store_true');a=ap.parse_args();root=Path(a.root).resolve();catalog=json.loads((root/a.catalog).read_text(encoding='utf-8'))
 publish_json=root/'Tests/Evidence/HSB_2D_V1_R3_MUTATION_RESULTS.json';publish_text=root/'Tests/Evidence/HSB_2D_V1_R3_MUTATION_RESULTS.txt';outj=Path(a.output_json).resolve() if a.output_json else (publish_json if a.publish_evidence else None);outt=Path(a.output_text).resolve() if a.output_text else (publish_text if a.publish_evidence else None)
 required={m['id'] for m in catalog if m.get('required')};ids=[m['id'] for m in catalog];infra=[]
 if len(ids)!=len(set(ids)):infra.append('duplicate_catalog_id')
 if not required<=set(ids):infra.append('required_missing')
 verifier=root/'Tests/Static/verify_hsb_2d_v1_r3.py';clean=subprocess.run([sys.executable,str(verifier),'--root',str(root)],text=True,capture_output=True)
 ...
 for m in catalog:
  if not m.get('required'):continue
 ...
 lines+=['HSB_2D_V1_R3_MUTATION_SUMMARY']+[f'{k}={v}' for k,v in summary.items()]+[f'RUNNER_SELF_TESTS={sum(x["status"]=="PASS" for x in selfcheck)}/10',f"RESULT={'PASS' if ok else 'FAIL'}"]
 text='\n'.join(lines)+'\n';
