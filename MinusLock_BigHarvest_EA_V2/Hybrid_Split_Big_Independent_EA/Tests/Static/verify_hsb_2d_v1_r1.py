...
def run(root,fixture,baseline):
 ...
 v.exact('S028',dec,['if(x.stateRevision!=revision)return HSBI_RuntimeReject(x,HSBI_DECISION_CONFLICT,HSBI_RD_STATE_REVISION_MISMATCH'],['false && x.stateRevision','if(false)','x.stateRevision==revision','true || x.stateRevision','r.valid=true;return r;if(x.stateRevision','early.valid=true;return early;'])
 ...
 v.exact('S038R',rst,['if(!s.snapshotPresent||s.persistedDigest=="")return HSBI_RuntimeReject','if(s.unresolvedPending)return HSBI_RuntimeReject(s.current,HSBI_DECISION_PERSISTENCE_REQUIRED'],['if(false && !s.snapshotPresent','if(false && s.unresolvedPending'])
 ...
 v.add('S040',not false_status,'no false status');v.add('S041','HSB.2E=NOT_STARTED' in docs,'2E');v.add('S042','TRADING_IMPLEMENTED=NO' in docs,'trading');v.add('S043','REAL_TRADING_ALLOWED=NO' in docs,'real');v.add('S044',all('HSB_2D_V1_R1_CANONICAL_STATUS_BEGIN' in v.read(x) for x in ['README_RU.md','BUILD_INFO.md','PROJECT_MAP_RU.md']),'canonical marker in every status document')
 manifest(v)
