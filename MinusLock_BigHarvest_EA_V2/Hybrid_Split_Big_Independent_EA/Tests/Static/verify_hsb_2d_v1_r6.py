...
BASELINE='202b36e454bbee592bfae9a20226f67d0d4fbbae';PREFIX='MinusLock_BigHarvest_EA_V2/Hybrid_Split_Big_Independent_EA/'
...
REQUIRED={'CURRENT_STAGE':'HSB.2D-V1-R6','HSB.2D_V1_R1_PREVIOUS_ACCEPTANCE':'HISTORICAL_SUPERSEDED','HSB.2D_V1_R2_PREVIOUS_ACCEPTANCE':'HISTORICAL_SUPERSEDED','HSB.2D_V1_R3_PREVIOUS_ACCEPTANCE':'HISTORICAL_SUPERSEDED','HSB.2D_V1_R4_PREVIOUS_ACCEPTANCE':'HISTORICAL_SUPERSEDED','HSB.2D_V1_R5_PREVIOUS_ACCEPTANCE':'HISTORICAL_SUPERSEDED','HSB.2D_V1_R6':'CORRECTED_OFFLINE_VERIFICATION','NO_OP_SCOPE_PROOF':'PASS','NEGATED_CONDITION_NORMALIZATION':'PASS','GUARD_OUTCOME_DOMINANCE':'PASS','HSB.2D_V2_HANDOFF':'READY','HSB.2E_PREP_R1':'IMPLEMENTATION_READY_FOR_ADMIN_REVIEW','METAEDITOR_MAIN_COMPILE':'NOT_EXECUTED_MT5_UNAVAILABLE','METAEDITOR_TEST_COMPILE':'NOT_EXECUTED_MT5_UNAVAILABLE','MQL5_TESTS_T01_T464':'NOT_EXECUTED_MT5_UNAVAILABLE','BROKER_MONEY_RUNTIME_PROOF':'NOT_EXECUTED_MT5_UNAVAILABLE','HSB.2D_V2':'AWAITING_ADMIN_REVIEW','HSB.2E':'NOT_STARTED','TRADING_LOGIC_START_ALLOWED':'NO','BROKER_DISPATCH_IMPLEMENTED':'NO','TRADE_REQUESTS_ALLOWED':'NO','REAL_TRADING_ALLOWED':'NO'}
SEAL_FILES=('Tests/Evidence/HSB_2D_V1_R6_CLEAN_RESULT.txt','Tests/Evidence/HSB_2D_V1_R6_MUTATION_RESULTS.json','Tests/Evidence/HSB_2D_V1_R6_MUTATION_RESULTS.txt','Tests/Static/verify_hsb_2d_v1_r6.py','Tests/Static/run_hsb_2d_v1_r6_mutations.py','Tests/Static/hsb_2d_v1_r6_mutations.json','Tests/Static/hsb_mql5_lexer.py','Tests/Evidence/HSB_2D_V1_R6_GUARD_OUTCOME_PROOFS.json','Tests/Evidence/HSB_2D_V1_R6_RETURN_PATHS.json','Tests/Evidence/HSB_2D_V1_R6_NO_OP_AUTHORIZATION.json','Tests/Evidence/HSB_2D_V1_R6_CONDITION_NORMALIZATION.json','Tests/Evidence/HSB_2D_V1_R6_ADVERSARIAL_RESULTS.json','Tests/Static/verify_hsb_2e_prep_r1.py','Tests/Static/hsb_2e_api_contracts.json','Tests/Static/hsb_2e_production_file_map.json','Tests/Static/hsb_2e_fsm_transitions.json','Tests/Static/hsb_2e_persistence_schema.json','Tests/Static/hsb_2e_transaction_lifecycle.json','Tests/Static/hsb_2e_fixtures.json','Tests/Static/hsb_2e_test_plan.json','Reports/HSB_2E_PREP_R1_COMPLETENESS_AUDIT_RU.md')
...
def expected_files(r):
 f={'Hybrid_Split_Big_Independent_EA.mq5','Tests/MQL5/HSBI_Skeleton_Tests.mq5','Tests/Static/verify_hsb_2d_v1_r6.py','Tests/Static/run_hsb_2d_v1_r6_mutations.py','Tests/Static/hsb_2d_v1_r6_mutations.json','Tests/Static/verify_hsb_2d_v2_user_evidence.py','Tests/Static/verify_hsb_2e_prep_r1.py','Tests/Static/hsb_mql5_lexer.py','Tests/Static/hsb_2e_expected_dependency_graph.json','Tests/Static/hsb_2e_api_contracts.json','Tests/Static/hsb_2e_production_file_map.json','Tests/Static/hsb_2e_fsm_transitions.json','Tests/Static/hsb_2e_persistence_schema.json','Tests/Static/hsb_2e_transaction_lifecycle.json','Tests/Static/hsb_2e_global_invariants.json','Tests/Static/hsb_2e_fixtures.json','Tests/Static/hsb_2e_test_plan.json',*STATUS_DOCUMENTS}
 f|={p.relative_to(r).as_posix() for p in r.glob('Include/**/*.mqh')};f|={p.relative_to(r).as_posix() for p in r.glob('Reports/HSB_2D_V1_R6_*.md') if 'FINAL_PUBLICATION_RECORD' not in p.name};f|={p.relative_to(r).as_posix() for p in r.glob('Docs/HSB_2*.md')};f|={p.relative_to(r).as_posix() for p in r.glob('Reports/HSB_2E_*.md')};f|={p.relative_to(r).as_posix() for p in r.glob('Reports/HSB_2D_V2_*.md')};return f
...
def status(r):
 ...
 for rel in STATUS_DOCUMENTS:
  s=(r/rel).read_text(encoding='utf-8-sig');bm='HSB_2D_V1_R6_CANONICAL_STATUS_BEGIN';em='HSB_2D_V1_R6_CANONICAL_STATUS_END';meta[rel]=(s.count(bm),s.count(em));
...
def adversarial_cases():
 statuses=('0','(HSBI_RuntimeDecisionStatus)0','alias','c?HSBI_DECISION_REJECTED:HSBI_DECISION_VALID','GetStatus()','HSBI_DECISION_VALID','HSBI_DECISION_NO_OP')
 cases=[]
 ...
 s='R HSBI_ValidateRestartedRuntimeState(){if(s.duplicateConsumption)return HSBI_RuntimeReject(x,HSBI_DECISION_NO_OP,HSBI_RD_OK,"n");}';cases.append({'id':'A015','dimension':'AUTHORIZED_NO_OP','input':'S037','caught':prove_top_level_guard(s,'HSBI_ValidateRestartedRuntimeState','s.duplicateConsumption','HSBI_DECISION_NO_OP','HSBI_RD_OK')['RESULT']=='PASS'})
 return cases
def run(root,fixture=False,skip_seal=False):
 ...
 mok,md,mm=check_list(r,'Reports/HSB_2D_V1_R6_FILE_MANIFEST_SHA256.txt',expected_files(r));add('S045',mok,md);sok,sd,sm=check_list(r,'Tests/Evidence/HSB_2D_V1_R6_EVIDENCE_SEAL_SHA256.txt',set(SEAL_FILES));sm=({k:0 for k in ('MISSING','EXTRA','DUPLICATES','EXTERNAL_PATHS','HASH_MISMATCHES')} if skip_seal else sm);add('S046E',sok or skip_seal,'evidence seal verified' if sok or skip_seal else sd);lex=lexer_self_tests();add('SLEX65',len(lex)>=65 and all(lex.values()),f'passed={sum(lex.values())}/{len(lex)}')
...
def main():
 ...
 for path,data in ((a.return_output,ret),(a.noop_output,payload['no_op_authorization']),(a.normalization_output,payload['condition_normalization']),(a.adversarial_output,adv)):
