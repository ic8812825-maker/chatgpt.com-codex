...
def main():
 ...
 with tempfile.TemporaryDirectory(prefix='r4-r1-mut-') as td:
  ...
  for spec in cat:
   ...
   mut=target.read_bytes();shutil.rmtree(f/'Tests/Reference/__pycache__',ignore_errors=True);shutil.rmtree(f/'Tests/Static/__pycache__',ignore_errors=True);cp=subprocess.run([sys.executable,str(f/'Tests/Static/verify_hsb_2e_prep_r4_r1.py'),'--root',str(f),'--fixture-mode','--skip-integrity'],capture_output=True,text=True,timeout=30);failed=[z.split('|',1)[0] for z in cp.stdout.splitlines() if '|FAIL' in z];expected=spec['EXPECTED_CHECK_IDS'];observed=any(any(x==e or x.startswith(e) for x in failed) for e in expected);normal='RESULT=FAIL' in cp.stdout
