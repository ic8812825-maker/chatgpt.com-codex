#!/usr/bin/env python3
"""Fresh R10 cumulative regression catalog; evidence is output only when explicitly requested."""
...
OUT=v.ROOT/'Tests/Evidence/R4A_R10/regression_results.json'
def classify(fn):
 try:fn()
...
def run(fs=None):
 ...
 add('FOREIGN_REPLAY_IDS','NORMATIVE_REJECTION','R10_REPLAY_STATE_BINDING','HISTORICAL_OUTPUT_PERSISTED_STATE_MISMATCH',mut(rep,badrep))
 ...
 add('HISTORICAL_REVISION_REVERSED','NORMATIVE_REJECTION','R10_REPLAY_REVISION_BINDING','HISTORICAL_REVISION_DOMAIN_MISMATCH',mut(rep,histrev))
 ...
 add('CERT_PROVIDED_PRECOMMIT','NORMATIVE_REJECTION','R9_SCENARIO_PHASE','PRE_COMMIT_CURRENT_EVIDENCE_FORBIDDEN',mut(next(x['scenarioInput'] for x in runs if x['scenarioInput']['phase']=='PRE_COMMIT'),lambda x:x.__setitem__('certificate',copy.deepcopy(comm['certificate']))))
 ...
 add('PHASE_DOWNGRADE_WITH_EVIDENCE','NORMATIVE_REJECTION','R9_SCENARIO_PHASE','PRE_COMMIT_CURRENT_EVIDENCE_FORBIDDEN',mut(comm,downgrade))
 ...
 add('REPLAY_ARBITRARY_CURRENT_REVISION','NORMATIVE_REJECTION','R10_REPLAY_REVISION_BINDING','HISTORICAL_OUTPUT_CURRENT_REVISION_MISMATCH',mut(rep,current999))
 ...
 def setboth(x,key,value):
  for z in x['deals']+x['events']:z[key]=value
  reseal(x)
 add('UNCONFIRMED_RECORDS','NORMATIVE_REJECTION','R9_EXECUTION_CONFIRMATION','EXECUTION_NOT_CONFIRMED',mut(comm,lambda x:setboth(x,'confirmed',False)))
 add('DEAL_STATE_REVISION','NORMATIVE_REJECTION','R9_EXECUTION_REVISION','STATE_REVISION_CONTEXT_MISMATCH',mut(comm,lambda x:(x['deals'][0].__setitem__('stateRevision',999),reseal(x))))
 add('EVENT_STATE_REVISION','NORMATIVE_REJECTION','R9_EXECUTION_REVISION','STATE_REVISION_CONTEXT_MISMATCH',mut(comm,lambda x:(x['events'][0].__setitem__('stateRevision',999),reseal(x))))
 add('DEAL_SNAPSHOT_REVISION','NORMATIVE_REJECTION','R9_EXECUTION_REVISION','SNAPSHOT_REVISION_CONTEXT_MISMATCH',mut(comm,lambda x:(x['deals'][0].__setitem__('snapshotRevision',999),reseal(x))))
 add('EVENT_SNAPSHOT_REVISION','NORMATIVE_REJECTION','R9_EXECUTION_REVISION','SNAPSHOT_REVISION_CONTEXT_MISMATCH',mut(comm,lambda x:(x['events'][0].__setitem__('snapshotRevision',999),reseal(x))))
 add('SCENARIO_PHASE_MISMATCH','NORMATIVE_REJECTION','R9_SCENARIO_PHASE','REPLAY_SCENARIO_REQUIRES_REPLAY',mut(comm,lambda x:(x.__setitem__('scenario','REPLAY_COMMITTED'),reseal(x))))
 add('MISSING_CONTEXT_ACCOUNT','NORMATIVE_REJECTION','R5_SCHEMA','MISSING_REQUIRED',mut(comm,lambda x:x['context'].pop('accountId')))
 def large(x):
  n=9007199254740993;x['context']['magic']=n;x['snapshot']['magic']=n
  for z in x['positions']+x['deals']+x['events']:z['magic']=n
  reseal(x)
 add('LARGE_EXACT_IDENTIFIER','ACCEPTED','','',mut(comm,large))
 def metadata_modified():
  z=copy.deepcopy(fs);z[0]['testContract']={'changed':'arbitrary'};return v.execute(z)
 def metadata_removed():
  z=copy.deepcopy(fs)
  for f in z:f.pop('testContract',None)
  return v.execute(z)
 add('METADATA_MODIFIED','ACCEPTED','','',metadata_modified)
 add('METADATA_REMOVED','ACCEPTED','','',metadata_removed)
 def foreign_history(key,value):
  def change(x):
   h=x['replayContract']['historicalSourceObjects'];h['context'][key]=value;x['certificate']=cert(h)
  return change
 for cid,key,value in [('REPLAY_FOREIGN_ACCOUNT','accountId','FOREIGN-ACCOUNT'),('REPLAY_FOREIGN_SYMBOL','symbol','GBPUSD'),('REPLAY_FOREIGN_MAGIC','magic',999999),('REPLAY_FOREIGN_CYCLE','cycleId','FOREIGN-CYCLE'),('REPLAY_FOREIGN_TRANSACTION','transactionId','FOREIGN-TX'),('REPLAY_FOREIGN_ACTION','actionId','FOREIGN-ACT')]:
  add(cid,'NORMATIVE_REJECTION','R10_REPLAY_SOURCE_BINDING','HISTORICAL_CURRENT_IDENTITY_MISMATCH',mut(rep,foreign_history(key,value)))
 def unbound_state(x):
  h=x['replayContract']['historicalSourceObjects'];h['persistedState']['reserve']='999';x['certificate']=cert(h)
 add('REPLAY_UNBOUND_PERSISTED_OUTPUT','NORMATIVE_REJECTION','R10_REPLAY_STATE_BINDING','HISTORICAL_OUTPUT_PERSISTED_STATE_MISMATCH',mut(rep,unbound_state))
 add('VALID_REPLAY_SAME_OPERATION','ACCEPTED','','',lambda:v.runtime(copy.deepcopy(rep)))
 accumulated=next(x['scenarioInput'] for x in runs if x['scenarioInput']['phase']=='REPLAY' and any(z.startswith('HISTORY-') for z in x['scenarioInput']['persistedState']['consumedDealIds']))
 add('VALID_REPLAY_ACCUMULATED_HISTORY','ACCEPTED','','',lambda:v.runtime(copy.deepcopy(accumulated)))
 rows=[]
