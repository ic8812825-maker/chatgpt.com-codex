...
class EconomicExecutionResult:
 projected_money:D;realized_cycle_net:D;recovery_pl_close_now:D;source_pool_net:D;allocations:D;consumptions:D;residual:D;digest:EconomicStateDigest;final_close_allowed:bool;reason_codes:tuple[str,...];deal_applications:int;event_state:str;facts:'EconomicFacts'
@dataclass(frozen=True)
class EconomicFacts:
 projected_reference:D;realized_reference:D;eligible_deal_nets:tuple[D,...];source_deal_nets:tuple[D,...];allocation_amounts:tuple[D,...];allocation_residuals:tuple[D,...];allocation_consumed:tuple[D,...];planned_allocation:D;planned_residual:D;event_state_allowed:bool;deal_tickets_unique:bool;transaction_ids_unique:bool;persistence_roundtrip:bool
...
def execute_scenario(x:EconomicScenarioInput=EconomicScenarioInput())->EconomicExecutionResult:
 ...
 if x.defect_operation=='RESERVE_FOR_PARTIAL':
  try:allocation.consume(key,ConsumptionKey(1,'EURUSD',7,'C','PARTIAL_FAR',1,'POST','P','M1',ConsumptionPurpose.PARTIAL_FAR,key),D('1'))
  except ValueError:reasons.append('PURPOSE_ROUTE_BLOCKED')
 elif x.defect_operation=='DUPLICATE_EVENT_RESTART':
  restored=PersistentStore.deserialize(persisted);reasons.append('DUPLICATE_EVENT_BLOCKED' if key in restored.events else 'DUPLICATE_EVENT_LOST')
 elif x.defect_operation=='PARTIAL_FILL_RESIDUAL':
  cost=OpenPositionCost(D('1'),D('-10'));cost.close(D('.5'),D('.4'),1);reasons.append('PARTIAL_RESIDUAL_PRESENT' if cost.unallocated_entry_cost==D('-6') else 'PARTIAL_RESIDUAL_LOST')
 elif x.defect_operation=='DEPOSIT':
  accepted=ledger.apply(Deal(ident,99,'BALANCE',DealEntry.IN,DealType.BALANCE,D('.01'),D('100')));reasons.append('DEPOSIT_INCLUDED' if accepted else 'DEPOSIT_BLOCKED')
 elif x.defect_operation=='INITIAL_IGNORED':
  accepted=ledger.apply(Deal(ident,98,'P',DealEntry.OUT,DealType.BUY,D('.01'),D('100'),initial_ignored=True));reasons.append('INITIAL_INCLUDED' if accepted else 'INITIAL_BLOCKED')
 ...
 facts=EconomicFacts(projected_profit(x.side,x.intended_volume,open_price,broker),intended_realized,tuple(d.net for d in ledger.closing_deals()),tuple(p.aggregate_actual_source_net for p in allocation.source_pools.values()),tuple(r.amount for r in allocation.records.values()),tuple(r.residual for r in allocation.records.values()),tuple(r.consumed for r in allocation.records.values()),x.intended_allocation,x.intended_residual,event.state is ReconciliationState.RECONCILED,len(ledger.deals)==len(set(ledger.deals)),len({k.transaction_id for k in allocation.consumptions})==len(allocation.consumptions),persisted==PersistentStore.deserialize(persisted).serialize())
 return EconomicExecutionResult(projected,realized,realized,pool_net,allocation.available(AllocationType.FINAL_RESERVE),sum((r.amount for r in allocation.consumptions.values()),D('0')),x.residual,digest,not reasons,tuple(reasons),applied,event.state.value,facts)
@dataclass(frozen=True)
