...
bool CompleteSmallOperationAudit(int legRole,double residualLot)
{
   ...
   if(IsInternalSimulationMode()) { for(int i=0;i<ArraySize(SimClosedDeals);i++) if(SimClosedDeals[i].positionTicket==a.ticket && SimClosedDeals[i].positionIdentifier==a.identifier) { filled+=SimClosedDeals[i].lot; net+=SimClosedDeals[i].netMoney; commission+=SimClosedDeals[i].commission; swap+=SimClosedDeals[i].swap; fee+=SimClosedDeals[i].fee; if(first==0)first=(long)SimClosedDeals[i].ticket;last=(long)SimClosedDeals[i].ticket; } }
