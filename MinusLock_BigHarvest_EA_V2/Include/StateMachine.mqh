...
bool CompleteSmallOperationAudit(int legRole,double residualLot)
{
   ...
   if(IsInternalSimulationMode()) { for(int i=0;i<ArraySize(SimDeals);i++) if(SimDeals[i].positionTicket==a.ticket && SimDeals[i].positionIdentifier==a.identifier) { filled+=SimDeals[i].lot; net+=SimDeals[i].netMoney; commission+=SimDeals[i].commission; swap+=SimDeals[i].swap; fee+=SimDeals[i].fee; if(first==0)first=(long)SimDeals[i].ticket;last=(long)SimDeals[i].ticket; } }
