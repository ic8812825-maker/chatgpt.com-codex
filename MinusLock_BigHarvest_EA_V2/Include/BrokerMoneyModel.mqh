...
struct SmallTransitionLeg { SmallTransitionLegRole role; BrokerMoneyResult money; double actualPositionLot; double requestedLot; double openLot; double closeLot; double residualLot; bool fullClose; bool includesOpenAndClose; };
...
struct ReverseCycleProjection { double farLotBefore; double bigCoreLot; double bigTrendLot; double smallBaseLot; double reverseSmallLot; double targetNewFar; double closeLot; double farLotAfter; double farLossBefore; double farLossAfter; double transitionNet; double signedSwap; double commission; double spread; double slippage; double reserveAdd; double carryAdd; double requiredMargin; double availableMargin; };
...
bool EvaluateSmallTransition(SmallTransitionLeg &legs[],double oldFarLot,double projectedNewFarLot,double netSmallExposure,double marginLevel,SmallTransitionEvaluation &e)
{
   ...
   int trendIndex=-1,baseIndex=-1; for(int j=0;j<5;j++){if(legs[j].role==SMALL_LEG_BIG_TREND_CLOSE)trendIndex=j;if(legs[j].role==SMALL_LEG_SMALL_BASE_CLOSE)baseIndex=j;}
   double tolerance=MathMax(VolumeMismatchToleranceLots,SymbolInfoDouble(_Symbol,SYMBOL_VOLUME_STEP)*0.5);
   bool trendExact=trendIndex>=0&&legs[trendIndex].fullClose&&MathAbs(legs[trendIndex].requestedLot-legs[trendIndex].actualPositionLot)<=tolerance&&legs[trendIndex].residualLot<=tolerance;
   bool baseExact=baseIndex>=0&&legs[baseIndex].fullClose&&MathAbs(legs[baseIndex].requestedLot-legs[baseIndex].actualPositionLot)<=tolerance&&legs[baseIndex].residualLot<=tolerance;
   bool reverseExact=reverseIndex>=0&&legs[reverseIndex].includesOpenAndClose&&MathAbs(legs[reverseIndex].openLot-legs[reverseIndex].closeLot)<=tolerance&&MathAbs(legs[reverseIndex].openLot-legs[reverseIndex].actualPositionLot)<=tolerance&&legs[reverseIndex].residualLot<=tolerance;
   bool farExact=oldFarIndex>=0&&legs[oldFarIndex].fullClose&&MathAbs(legs[oldFarIndex].requestedLot-oldFarLot)<=tolerance&&legs[oldFarIndex].residualLot<=tolerance;
   bool coreExact=coreIndex>=0&&MathAbs(legs[coreIndex].requestedLot-(legs[coreIndex].actualPositionLot-projectedNewFarLot))<=tolerance&&MathAbs(legs[coreIndex].residualLot-projectedNewFarLot)<=tolerance;
   if(!trendExact||!baseExact||!reverseExact||!farExact||!coreExact) { e.calculationValid=false; e.transitionAllowed=false; e.reason="SMALL_LEG_LOT_CONTRACT_INVALID"; return false; }
...
bool EvaluateBigBasketGate(Direction directions[],double lots[],int managedPositions,BigBasketGate &g)
{
   ...
   for(int i=0;i<3;i++)
   {
      ...
      double openPrice=BrokerExecutionOpenPrice(directions[i]); BrokerMoneyResult m; if(!CalcProjectedMarginMoney(directions[i]==DIR_BUY?ORDER_TYPE_BUY:ORDER_TYPE_SELL,lots[i],openPrice,m)) { g.reason="BIG_BASKET_MARGIN_CALC_FAILED"; return false; } g.newBasketMargin+=m.requiredMargin;
...
{
   e.requiredCycles=0; e.finalFarLot=farLot; e.finalFarLoss=farLoss; e.projectedReserve=reserve; e.projectedCarry=carry; e.projectedRecoveryPL=recovery; e.finalCoverage=farLoss>0?(reserve+carry)/farLoss:1; e.pass=false; e.reason="REVERSE_LIMIT";
   double ratio=cycle.farLotBefore>0?cycle.farLotAfter/cycle.farLotBefore:0;if(ratio<=0||ratio>=1||cycle.requiredMargin>cycle.availableMargin) { e.reason="REVERSE_CYCLE_INPUT_OR_MARGIN_FAIL"; return false; }
   for(int n=0;n<=MaxReverseCycles;n++) { e.requiredCycles=n; e.finalCoverage=e.finalFarLoss>0?(e.projectedReserve+e.projectedCarry)/e.finalFarLoss:1; if(e.finalFarLot<=targetLot&&e.finalCoverage>=1&&e.projectedRecoveryPL>=MinimumRecoveryProfitMoney) { e.pass=true; e.reason="OK"; return true; } if(n==MaxReverseCycles) break; e.finalFarLot=NormalizeLotDown(e.finalFarLot*ratio); e.finalFarLoss*=ratio; e.projectedReserve+=cycle.reserveAdd; e.projectedCarry+=cycle.carryAdd; e.projectedRecoveryPL+=cycle.transitionNet+cycle.signedSwap-cycle.commission-cycle.spread-cycle.slippage; }
   return false;
}

bool EvaluateDynamicReverseCycles(double farLot,double farLoss,double reserve,double carry,double recovery,double targetLot,ReverseCycleProjection &projections[],ReverseCyclesEvaluation &e)
{
   e.requiredCycles=0;e.finalFarLot=farLot;e.finalFarLoss=farLoss;e.projectedReserve=reserve;e.projectedCarry=carry;e.projectedRecoveryPL=recovery;e.finalCoverage=farLoss>0?(reserve+carry)/farLoss:1;e.pass=false;e.reason="DYNAMIC_REVERSE_LIMIT";
   int count=ArraySize(projections);if(count==0||count>MaxReverseCycles){e.reason="DYNAMIC_PROJECTIONS_INVALID";return false;}
   for(int i=0;i<count;i++){ReverseCycleProjection p=projections[i];double tol=MathMax(VolumeMismatchToleranceLots,SymbolInfoDouble(_Symbol,SYMBOL_VOLUME_STEP)*.5);if(MathAbs(p.farLotBefore-e.finalFarLot)>tol||p.farLotAfter<=0||p.farLotAfter>=p.farLotBefore||p.farLossBefore<0||p.farLossAfter<0||p.requiredMargin>p.availableMargin){e.reason="DYNAMIC_CYCLE_INVALID";return false;}e.finalFarLot=p.farLotAfter;e.finalFarLoss=p.farLossAfter;e.projectedReserve+=p.reserveAdd;e.projectedCarry+=p.carryAdd;e.projectedRecoveryPL+=p.transitionNet+p.signedSwap-p.commission-p.spread-p.slippage;e.requiredCycles=i+1;e.finalCoverage=e.finalFarLoss>0?(e.projectedReserve+e.projectedCarry)/e.finalFarLoss:1;if(e.finalFarLot<=targetLot&&e.finalCoverage>=1&&e.projectedRecoveryPL>=MinimumRecoveryProfitMoney&&e.finalFarLoss<=e.projectedReserve+e.projectedCarry){e.pass=true;e.reason="OK";return true;}}
   return false;
}
