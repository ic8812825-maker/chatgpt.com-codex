...
class CFSMCompute
  {
public:
   static FSM_DTO ComputeState(const SystemState &system_state,const FlowSnapshot &snapshot)
     {
      FSM_DTO dto;
      dto.current=system_state.fsm_state;
      dto.next=system_state.fsm_state;

      if(CFSMSafeRules::ShouldEnterSafeMode(snapshot))
      ...
      if(!CFSMSafeRules::IsSafeTransition(dto.current,dto.next))
