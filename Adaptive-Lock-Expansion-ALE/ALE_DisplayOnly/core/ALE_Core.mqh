...
class CALECore
  {
public:
   static void Recalculate(SystemState &system_state,DualState &dual_state,const FlowSnapshot &input_snapshot)
     {
      FlowContext buy_ctx;
      ...
      CALEDualCycleEngine::RunDualCycle(buy_ctx,sell_ctx,input_snapshot,dual_state.buy_snapshot,dual_state.sell_snapshot);

      const FSM_DTO fsm=CFSMCompute::ComputeState(system_state,input_snapshot);
