...
bool TestALE_DualFlowIntegration()
{
   ...
   // I1 numerical derivative check dPnL/dP = Delta for buy stream linear book
   const double h=1e-5;
   ...
   // Test 6 SAFE trigger oracle
   if(!ale.AddVirtual(ALE_FLOW_BUY,1.0,0.1)) { /* allowed to fail in SAFE */ }

   // Test 7 illegal SAFE -> EXPANSION transition
   CALStateMachine fsm;
   if(!fsm.Transition(ALE_STATE_SAFE)) return false;
   if(fsm.Transition(ALE_STATE_EXPANSION)) return false;

   // Test 8 harvest condition oracle (state reachability through signal)
   ...
   // Test 9 edge margin safety oracle
   if(!(market.MarginRequired(1.0)==1000.0)) return false;

   // Test 10 zero equity safety oracle
