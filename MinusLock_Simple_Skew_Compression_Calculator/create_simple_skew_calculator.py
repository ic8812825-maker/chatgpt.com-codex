...
def build_tests(ws):
    ...
    rows = [
        ('База Main 165', "='Калькулятор'!B44", 165),
        ('База Opp 175', "='Калькулятор'!B45", 175),
        ('База Skew 10', "='Калькулятор'!B46", 10),
        ('Close 60', "='Калькулятор'!N26", 60),
        ('Close 30', "='Калькулятор'!N27", 30),
        ('Close 0', "='Калькулятор'!N28", 0),
        ('Сумма Big', "='Калькулятор'!B64", "='Калькулятор'!$B$2*1.55"),
        ('Сумма Small', "='Калькулятор'!B65", "='Калькулятор'!$B$2*0.75"),
        ('Сумма Close', "='Калькулятор'!B66", "='Калькулятор'!$B$2*0.90"),
        ('Финальный Rounded Main', "='Калькулятор'!B70", "='Калькулятор'!$B$2*1.65"),
        ('Финальный Rounded Opp', "='Калькулятор'!B71", "='Калькулятор'!$B$2*1.75"),
        ('Финальный Rounded Skew', "='Калькулятор'!B72", "='Калькулятор'!$B$2*0.10"),
        ('Максимальная маржа', "='Калькулятор'!B73", "='Калькулятор'!$B$2*3740"),
        ('Максимальная нагрузка %', "='Калькулятор'!B75", "='Калькулятор'!$B$2*37.4"),
        ('Риск-анализ синхронизирован', "=IF(AND(ABS('РИСК_АНАЛИЗ'!B7-'Калькулятор'!R22)<0.000001,ABS('РИСК_АНАЛИЗ'!C7-'Калькулятор'!Q22)<0.000001,ABS('РИСК_АНАЛИЗ'!D7-'Калькулятор'!N22)<0.000001,ABS('РИСК_АНАЛИЗ'!E7-'Калькулятор'!T22)<0.000001,ABS('РИСК_АНАЛИЗ'!F7-'Калькулятор'!X22)<0.000001,'РИСК_АНАЛИЗ'!G7='Калькулятор'!AA22),\"YES\",\"NO\")", 'YES'),
        ('Human Summary есть', "='Калькулятор'!A55", 'ИТОГОВЫЙ ЧЕЛОВЕЧЕСКИЙ РАСЧЁТ ВСЕХ УРОВНЕЙ'),
        ('Human Comment есть', "='Калькулятор'!U57", ''),
    ]

    i = 2
    for n, fact, exp in rows:
        ws[f'A{i}'] = n
        ws[f'B{i}'] = fact
        ws[f'C{i}'] = exp
        if isinstance(exp, (int, float)) or (isinstance(exp, str) and exp.startswith('=')):
            ws[f'D{i}'] = f'=IF(ABS(B{i}-C{i})<0.000001,"PASS","FAIL")'
        elif exp == '':
            ws[f'D{i}'] = f'=IF(LEN(B{i})>0,"PASS","FAIL")'
        else:
