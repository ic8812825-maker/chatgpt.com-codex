...
def build_calc(ws):
    ...
    params = [
        ("StartLot", 1.00, "стартовый лот"), ("StepPoints", 100, "шаг сетки в пунктах"),
        ("MaxLevels", 5, "максимум уровней"), ("LotStep", 0.01, "шаг лота брокера"),
        ("Direction", "DOWN", "выбранный сценарий DOWN/UP"), ("UseRounding", True, "использовать округление"),
        ("BigRoundMode", "DOWN", "Big округлять вниз"), ("SmallRoundMode", "UP", "Small округлять вверх"),
        ("CloseRoundMode", "SAFE", "защитное округление Close"), ("PointValue", 1, "стоимость пункта"),
        ("Spread", 0, "спред"), ("Commission", 0, "комиссия"),
    ]
    for i, (n, v, d) in enumerate(params, 2):
        ws[f"A{i}"] = n; ws[f"B{i}"] = v; ws[f"C{i}"] = d
    ...
    def table(start, label):
        ...
        for i,r in enumerate(range(start+2,start+7), start=18):
            ...
            ws[f"W{r}"]=f"=MAX(0,$B$2-P{r})+SUM($G${start+2}:G{r})"; ws[f"X{r}"]=f"=$B$2+SUM($I${start+2}:I{r})"; ws[f"Y{r}"]=f"=X{r}-W{r}"
            ws[f"Z{r}"]=(f"=IF(COUNTIF($F$2:$F$9,\"ERROR*\")>0,\"ERROR\"," \
                         f"IF(OR(B{r}<C{r},B{r}<0,C{r}<0,D{r}<0,E{r}<0,E{r}>J{r},T{r}>U{r},W{r}>X{r},Y{r}<0,AND(B{r}>0,G{r}=0),AND(C{r}>0,I{r}=0)),\"ERROR\"," \
                         f"IF(AND(D{r}>0,Y{r}<($B$2*D{r}/100)),\"WARNING\",\"OK\")))")

    table(24,"DOWN CALCULATION")
    table(33,"UP CALCULATION")

    title(ws,"A42","SUMMARY")
    rows=[("Selected Direction",'=B6'),("Final Total Main %",'=IF(B6="DOWN",T30,T39)'),("Final Total Opposite %",'=IF(B6="DOWN",U30,U39)'),
          ("Final Skew %",'=IF(B6="DOWN",V30,V39)'),("Final Start Remaining %",'=IF(B6="DOWN",Q30,Q39)'),("Final Rounded Main Lot",'=IF(B6="DOWN",W30,W39)'),
          ("Final Rounded Opp Lot",'=IF(B6="DOWN",X30,X39)'),("Final Rounded Skew Lot",'=IF(B6="DOWN",Y30,Y39)'),("Final Rounded Status",'=IF(B6="DOWN",Z30,Z39)'),
...
def build_tests(ws):
    ...
    rows=[("Down L1 Close%",'=Calculator!N26',60),("Down L2 Close%",'=Calculator!N27',30),("Down Final Main",'=Calculator!T30',165),
          ("Down Final Opp",'=Calculator!U30',175),("Down Final Skew",'=Calculator!V30',10),("Up L1 Close%",'=Calculator!N35',60),
          ("Up L2 Close%",'=Calculator!N36',30),("Up Final Main",'=Calculator!T39',165),("Up Final Opp",'=Calculator!U39',175),("Up Final Skew",'=Calculator!V39',10)]
    for i,(n,a,e) in enumerate(rows,2): ws[f"A{i}"]=n; ws[f"B{i}"]=a; ws[f"C{i}"]=e; ws[f"D{i}"]=f'=IF(B{i}=C{i},"PASS","FAIL")'


def build_manual(ws):
    ws.title="Manual"; ws["A1"]="Инструкция"; ws["A1"].font=Font(bold=True)
    lines=["1. Ввести StartLot.","2. Проверить StepPoints.","3. Выбрать Direction DOWN или UP.","4. При необходимости изменить Level Grid.","5. Смотреть Main Table.","6. Проверить Summary.","7. Если Status = OK — сетка безопасна по математике.","8. Если ERROR — использовать нельзя.","9. Если WARNING — проверить skew и rounded-лоты."]
    for i,l in enumerate(lines,2): ws[f"A{i}"]=l


def build_readme(ws):
    ws.title="README"; ws["A1"]="README"; ws["A1"].font=Font(bold=True)
    text=["Это простой калькулятор skew-компрессии минусового замка.","Big — крупный ордер на основной стороне.","Small — малый ордер на противоположной стороне.","Safe Close — безопасное частичное закрытие стартового ордера.","Total Main — суммарный объём основной стороны.","Total Opposite — суммарный объём противоположной стороны.","Main <= Opposite обязательно для сохранения защиты.","Status: OK — безопасно, WARNING — проверить skew/rounded, ERROR — нельзя использовать."]
    for i,t in enumerate(text,2): ws[f"A{i}"]=t
