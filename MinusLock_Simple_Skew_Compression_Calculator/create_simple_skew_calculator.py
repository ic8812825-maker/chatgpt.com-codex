...
def build_calc(ws):
    ...
    def table(start,label):
        ...
        for i,r in enumerate(range(start+2,start+7),start=18):
            ...
            ws[f"Z{r}"]=(f"=IF(COUNTIF($F$2:$F$9,\"ERROR*\")>0,\"ERROR\",IF(OR(B{r}<C{r},B{r}<0,C{r}<0,D{r}<0,E{r}<0,E{r}>J{r},T{r}>U{r},W{r}>X{r},AND(B{r}>0,G{r}=0),AND(C{r}>0,I{r}=0)),\"ERROR\",IF(AND(D{r}>0,Y{r}<($B$2*D{r}/100)),\"WARNING\",\"OK\")))")
